...
#	protected $signalSlotDispatcher;
	 ...
	 */
    private $ResetFrameworkCache = false;
...

{
	...
	public function run(){
		...
		// Trace
		$this->debug->trace('==> Start FrontController','FrontController'); 

		 /*
		  * Repeater function {do while} for interrupted the request prozess 
		  * if session time out, then waiting for user login
		  */ 
		 do	{

			// Scaning NSP_Application
	        if($this->registry->SystemFrontControllerRepeater && $this->registry->routerUsd === true){
			   $this->initializeApplicationUnityNamespace();	    


$f = $this->objectManager;

$f->destroy('View');
$f->destroy('unitView');
$f->destroy('PreControllerChain');
$f->destroy('PostControllerChain');
$f->destroy('PostControllerView');

/*
$f->destroy('PostControllerView');
$f->destroy('PostControllerHeader');
$f->destroy('PostControllerFooter');
$f->destroy('PostControllerTitle');
*/

#$o = $this->objectManager->getAllObjectNames();

#print_r($o);

				// Debug Trace
				$this->debug->trace('### Repeater started ###','Bootstrap');			
		    } 
			...
			$FrontController = $this->objectManager->getObject('FrontController');
    	    $FrontController->run();
			
			// destroy front controller
			unset($FrontController);
		}
		while ($this->registry->SystemFrontControllerRepeater);

/*	
		$request = $this->objectManager->getObject('Request');
		
		if($request->isAjax() || $request->isObligatoryAjax()){
		 
		 $controller = $request->getGet('ac');
		 $action = 'responseAction';

		 $ac = $this->objectManager->getObject($controller); 

		 if(is_callable(array($ac,$action))){
             $ac->$action();
          } 		 
...
#   	    $FrontController->run();
	#	}
	  	...
	  	// Trace
		$this->debug->trace('==> End FrontController','FrontController'); 
		$this->debug->trace('==> End Bootstrap','gc'); 		
		$this->debug->trace('***** End Application ','FLOWLite'); 
